...
from app.services.attachment_service import AttachmentDownload, AttachmentService
...
class BridgeService:
    def __init__(
        ...
        *,
        ...
        self._remote_fetch_guard = RemoteFetchGuard(
            settings=settings,
            http_client=http_client,
        )
    ...
    async def add_by_doi(self, payload: AddByDOIRequest) -> AddByDOIResponse:
        return await self._ingest_service.add_by_doi(payload)

    async def import_metadata(self, payload: ImportMetadataRequest) -> ImportMetadataResponse:
        return await self._ingest_service.import_metadata(payload)

    async def import_discovery_hit(
        self,
        payload: ImportDiscoveryHitRequest,
    ) -> ImportDiscoveryHitResponse:
        return await self._ingest_service.import_discovery_hit(payload)
    ...
    async def search_items(
        ...
        *,
        ...
        return await self._library_service.search_items(
            q=q,
            start=start,
            limit=limit,
    ...
    async def search_items_advanced(
        ...
        *,
        ...
        return await self._library_service.search_items_advanced(
            q=q,
            fields=fields,
            title=title,
            author=author,
    ...
    async def build_review_pack(self, payload: ReviewPackRequest) -> ReviewPackResponse:
        return await self._library_service.build_review_pack(payload)
    ...
    async def search_discovery(
        ...
        *,
        ...
        return await self._discovery_service.search_discovery(
            q=q,
            start=start,
            limit=limit,
            year_from=year_from,
            year_to=year_to,
            oa_only=oa_only,
            resolve_in_library=resolve_in_library,
    ...
    async def list_items(
        ...
        *,
        ...
        return await self._library_service.list_items(
            start=start,
            limit=limit,
            item_type=item_type,
            collection_key=collection_key,
    ...
    async def batch_get_items(
        ...
        *,
        ...
        return await self._library_service.batch_get_items(
            item_keys=item_keys,
            include_attachments=include_attachments,
            include_notes=include_notes,
        )
    ...
    async def resolve_items(
        ...
        *,
        ...
        return await self._library_service.resolve_items(
            doi=doi,
            title=title,
            item_type=item_type,
            collection_key=collection_key,
    ...
    async def merge_duplicate_items(
        ...
        *,
        ...
        primary_relation_map = self._normalize_relation_map(primary_data.get("relations"))
        merged_tags = primary_tags[:]
        merged_collections = primary_collections[:]
        merged_relation_map = {
            relation_name: values[:]
            for relation_name, values in primary_relation_map.items()
        }
        for duplicate_raw in ordered_duplicates:
            ...
            merged_relation_map = self._merge_relation_maps(
                merged_relation_map,
                self._normalize_relation_map(duplicate_data.get("relations")),
            )
        ...
        merged_relation_map = self._filter_relation_map(
            merged_relation_map,
            excluded_item_keys=[primary_item_key, *duplicate_keys],
        )
        primary_relation_payload = self._relation_map_to_payload(primary_relation_map)
        merged_relation_payload = self._relation_map_to_payload(merged_relation_map)
        relations_changed = merged_relation_payload != primary_relation_payload
        ...
        if dry_run:
            primary_item = primary_item.model_copy(
                update={
                    "tags": merged_tags,
                    "collectionKeys": merged_collections,
                    "relations": self._normalize_relations(merged_relation_payload),
                }
            )
        ...
        if relations_changed:
            update_payload["relations"] = merged_relation_payload
        if update_payload:
            await self._zotero.update_item(
    ...
    async def list_item_attachments(self, item_key: str) -> AttachmentListResponse:
        return await self._attachment_service.list_item_attachments(item_key)

    async def get_attachment_detail(self, attachment_key: str) -> AttachmentDetailResponse:
        return await self._attachment_service.get_attachment_detail(attachment_key)
    ...
    async def create_attachment_handoff(
        ...
        *,
        ...
        return await self._attachment_service.create_handoff(
    ...
    async def download_attachment_by_token(self, token: str) -> AttachmentDownload:
        return await self._attachment_service.download_attachment_by_token(token)

    async def list_item_notes(self, item_key: str) -> ItemNotesResponse:
        return await self._notes_service.list_item_notes(item_key)
    ...
    async def create_item_note(
        ...
        *,
        ...
        return await self._notes_service.create_item_note(item_key=item_key, payload=payload)

    async def get_note_detail(self, note_key: str) -> NoteDetailResponse:
        return await self._notes_service.get_note_detail(note_key)
    ...
    async def update_note(
        ...
        *,
        ...
        return await self._notes_service.update_note(note_key=note_key, payload=payload)

    async def delete_note(self, *, note_key: str) -> NoteDeleteResponse:
        return await self._notes_service.delete_note(note_key=note_key)
    ...
    async def upsert_ai_note(
        ...
        *,
        ...
        return await self._notes_service.upsert_ai_note(item_key=item_key, payload=payload)
    ...
    async def get_item_citation(
        ...
        *,
        ...
        return await self._library_service.get_item_citation(
            item_key=item_key,
            style=style,
            locale=locale,
            linkwrap=linkwrap,
    ...
    async def _collect_local_cache_search_refs(
        ...
        *,
        ...
        del q, seen, item_type, collection_key, tag, starting_ordinal
        return []
    ...
    async def _hydrate_search_result_refs_for_sort(self, refs: list[dict[str, Any]]) -> None:
        item_keys = sorted(
            {
                str(ref.get("itemKey") or "").strip()
                for ref in refs
                if not isinstance(ref.get("item"), SearchItem)
                and str(ref.get("itemKey") or "").strip()
            }
        )
        if not item_keys:
            return
        raw_items = await self._zotero.get_items_by_keys_raw(item_keys)
        raw_by_key = {
            str(raw_item.get("key") or ""): raw_item
            for raw_item in raw_items
            if str(raw_item.get("key") or "")
        ...
        for ref in refs:
            if isinstance(ref.get("item"), SearchItem):
                continue
            item_key = str(ref.get("itemKey") or "").strip()
            raw_item = raw_by_key.get(item_key)
            if raw_item is None:
                continue
            ref["item"] = await self._normalize_parent_item(
                raw_item,
                include_attachments=False,
                include_notes=False,
            )
    ...
    def _order_search_result_refs(
        ...
        *,
        ...
        def compare_refs(left: dict[str, Any], right: dict[str, Any]) -> int:
            return self._compare_search_result_refs(
                left,
                right,
                sort=sort,
        ...
        *,
        ...
        for tag in item.tags:
            if normalized_query in tag.casefold():
                self._append_search_hint(
                    hints,
                    field=self._tag_search_hint_field(tag),
                    snippet=tag,
                )
        ...
        *,
        ...
        if raw_item_type == "note":
            ...
            if note_snippet is not None:
                hints.append(
                    SearchHint(
                        field=self._note_search_hint_field(raw_hit),
    ...
    def _build_local_search_index_record(
        ...
        *,
        ...
        data = raw_item.get("data", {})
        ai_note_types: list[str] = []
        note_texts = [
            self._note_text_for_index(child)
            for child in children
            if child.get("data", {}).get("itemType") == "note"
        ]
        for child in children:
            ...
            if child_data.get("itemType") != "note":
                continue
            identity = self._note_renderer.extract_identity(
                self._normalize_tags(child_data.get("tags"))
            )
            if identity is None:
                continue
            _agent, note_type, _slot = identity
            if note_type not in ai_note_types:
                ai_note_types.append(note_type)
        item_key = str(raw_item.get("key") or "")
        return {
            "itemKey": item_key,
    ...
    async def _search_local_index_hits(
        ...
        *,
        ...
        for hit in hits:
            ...
            ai_note_types = [
                str(value)
                for value in record.get("aiNoteTypes", [])
                if isinstance(value, str) and value.strip()
            ]
            hints = [
                SearchHint(
                    field=(
                        f"aiNote:{ai_note_types[0]}"
                        if str(value.get("field") or "") == "note" and ai_note_types
    ...
    def _search_hint_weight(field: str) -> float:
        base_field = BridgeService._search_hint_field_key(field)
        return {
            "doi": 12.0,
            "title": 10.0,
            "creator": 7.0,
    ...
    def _append_search_hint(
        ...
        *,
        ...
        if snippet is None and field != "search":
            return
    ...
    def _build_advanced_search_result_item(
        ...
        *,
        ...
        for hint in extra_hints:
            if self._search_hint_matches_requested_fields(hint.field, fields):
                self._append_search_hint(
                    hints,
                    field=hint.field,
    ...
    async def upload_pdf_from_action(
        ...
        return await self._ingest_service.upload_pdf_from_action(payload)
    ...
    async def upload_pdf_bytes(
        ...
        *,
        ...
        return await self._ingest_service.upload_pdf_bytes(
            content=content,
            filename=filename,
    ...
    async def _search_candidate_items(self, query: str) -> list[dict[str, Any]]:
    ...
    @staticmethod
    def _search_hint_field_key(field: str) -> str:
        if field.startswith("tag:"):
            return "tag"
        if field.startswith("aiNote:"):
            return "aiNote"
        return field

    @classmethod
    def _search_hint_matches_requested_fields(cls, field: str, requested_fields: set[str]) -> bool:
        base_field = cls._search_hint_field_key(field)
        return base_field in requested_fields or (
            base_field == "aiNote" and "note" in requested_fields
        )

    @staticmethod
    def _tag_search_hint_field(tag: str) -> str:
        return f"tag:{tag}"

    def _note_search_hint_field(self, raw_note: dict[str, Any]) -> str:
        ...
        if identity is None:
            return "note"
        _agent, note_type, _slot = identity
        return f"aiNote:{note_type}"

    def _normalize_attachments(self, children: list[dict[str, Any]]) -> list[AttachmentRecord]:
    ...
    def _normalize_ai_notes(self, children: list[dict[str, Any]]) -> list[AINoteSummary]:
        ...
        for child in children:
            ...
            parsed_note = self._note_renderer.parse(str(data.get("note") or ""))
            notes.append(
                AINoteSummary(
                    ...
                    schemaVersion=parsed_note.schema_version,
                )
            )
        return notes
    ...
    def _normalize_note_record(self, raw_note: dict[str, Any]) -> NoteRecord:
        ...
        parsed_note = self._note_renderer.parse(body_html)
        return NoteRecord(
            noteKey=str(raw_note.get("key") or ""),
            itemKey=self._clean_optional_str(data.get("parentItem")),
            bodyHtml=parsed_note.human_html,
            bodyText=self._note_renderer.to_plain_text(parsed_note.human_html),
            ...
            slot=slot,
            schemaVersion=parsed_note.schema_version,
            structuredPayload=parsed_note.payload,
            provenance=parsed_note.provenance,
        )

    def _note_text_for_index(self, raw_note: dict[str, Any]) -> str | None:
        body_html = str(raw_note.get("data", {}).get("note") or "")
        parsed_note = self._note_renderer.parse(body_html)
        visible_text = self._normalize_search_text(
            self._note_renderer.to_plain_text(parsed_note.human_html)
        )
        structured_text = self._normalize_search_text(
            self._note_renderer.structured_payload_text(parsed_note.payload)
        )
        parts = [part for part in [visible_text, structured_text] if part]
        if not parts:
            return None
        return "\n\n".join(parts)
    ...
    @staticmethod
    def _normalize_relation_map(raw_relations: Any) -> dict[str, list[str]]:
        if not isinstance(raw_relations, dict):
            return {}
        ...
        for relation_name, relation_value in raw_relations.items():
            ...
            for value in values:
                if not isinstance(value, str) or not value or value in normalized_values:
                    continue
    ...
    def _merge_relation_maps(
        primary_relations: dict[str, list[str]],
        extra_relations: dict[str, list[str]],
    ) -> dict[str, list[str]]:
        merged = {
            relation_name: values[:]
            for relation_name, values in primary_relations.items()
        }
        for relation_name, values in extra_relations.items():
            merged_values = merged.setdefault(relation_name, [])
            for value in values:
    ...
    @classmethod
    def _filter_relation_map(
        cls,
        relation_map: dict[str, list[str]],
        *,
        excluded_item_keys: list[str],
    ) -> dict[str, list[str]]:
        excluded = {item_key for item_key in excluded_item_keys if item_key}
        filtered: dict[str, list[str]] = {}
        for relation_name, values in relation_map.items():
            filtered_values: list[str] = []
            for value in values:
                match = ZOTERO_RELATION_ITEM_PATTERN.search(value)
                if match is not None and match.group(1) in excluded:
                    continue
                filtered_values.append(value)
            if filtered_values:
                filtered[relation_name] = filtered_values
        return filtered

    @staticmethod
    def _relation_map_to_payload(relation_map: dict[str, list[str]]) -> dict[str, Any]:
        return {
            relation_name: values[:]
            for relation_name, values in relation_map.items()
            if values
    ...
    async def _create_metadata_item(self, payload: ImportMetadataRequest) -> str:
        template = await self._zotero.get_item_template(payload.itemType)
        item_payload = dict(template)
        item_payload["itemType"] = payload.itemType
        item_payload["title"] = payload.title
        if payload.abstractNote is not None:
            item_payload["abstractNote"] = payload.abstractNote
        if payload.publicationTitle is not None:
            item_payload["publicationTitle"] = payload.publicationTitle
        if payload.date is not None:
            item_payload["date"] = payload.date
        if payload.doi is not None:
            item_payload["DOI"] = payload.doi
        if payload.url is not None:
            item_payload["url"] = payload.url
        if payload.extra is not None:
            item_payload["extra"] = payload.extra
        item_payload["creators"] = self._creator_inputs_to_zotero(payload.creators)
        item_payload["tags"] = [{"tag": tag} for tag in payload.tags]
        collection_key = payload.collectionKey or self._settings.default_collection_key
        if collection_key:
    ...
    async def _update_imported_item(
        self,
        *,
        ...
        for field, value in (
            ...
            if value is not None and value != data.get(field):
                updated[field] = value
        creators = self._creator_inputs_to_zotero(payload.creators)
        if creators and creators != data.get("creators"):
            updated["creators"] = creators
        if payload.tags:
            existing_tags = self._normalize_tags(data.get("tags"))
            merged_tags = self._merge_strings(existing_tags, payload.tags)
            updated["tags"] = [{"tag": tag} for tag in merged_tags]
        if payload.collectionKey:
            ...
            updated["collections"] = self._merge_strings(
                existing_collections,
        ...
        await self._zotero.update_item(
            item_key=item_key,
            version=int(existing_item.get("version", 0)),
            data=updated,
        )
        await self._refresh_local_search_index_item(item_key)

    async def _find_item_by_weak_metadata_match(
        self,
        payload: ImportMetadataRequest,
    ) -> dict[str, Any] | None:
        normalized_title = self._normalize_title_key(payload.title)
        first_author = (
            self._creator_input_display_name(payload.creators[0])
            if payload.creators
            else None
        )
        expected_year = self._extract_year(payload.date)
        raw_items = await self._list_all_top_level_items_raw(
        ...
        for raw_item in raw_items:
            ...
            if self._normalize_title_key(str(data.get("title") or "")) != normalized_title:
                continue
            if expected_year and self._extract_year(data.get("date")) != expected_year:
                continue
            if first_author:
                creators = self._normalize_creators(data.get("creators"))
                if not creators:
                    continue
                if creators[0].displayName.casefold() != first_author.casefold():
                    continue
            return raw_item
        return None

    def _metadata_request_from_discovery_hit(
        self,
        payload: ImportDiscoveryHitRequest,
    ) -> ImportMetadataRequest:
        extra_parts: list[str] = []
        if payload.openAlexId:
            extra_parts.append(f"OpenAlex: {payload.openAlexId}")
        return ImportMetadataRequest(
            itemType=self._guess_item_type_from_discovery_hit(payload.workType),
            title=payload.title,
            creators=[
                self._discovery_author_to_creator(author)
                for author in payload.authors
                if author.name.strip()
            ],
            abstractNote=payload.abstract,
            publicationTitle=payload.venue,
            date=(
                payload.publicationDate
                or (str(payload.publicationYear) if payload.publicationYear else None)
            ),
            doi=payload.doi,
            ...
            updateIfExists=payload.updateIfExists,
            requestId=payload.requestId,
        )

    @staticmethod
    def _guess_item_type_from_discovery_hit(work_type: str | None) -> str:
        normalized = (work_type or "").strip().casefold()
        if normalized in {"book", "monograph"}:
            return "book"
        if normalized in {"proceedings", "conference-paper"}:
            return "conferencePaper"
        return "journalArticle"

    def _discovery_author_to_creator(self, author: DiscoveryAuthor) -> CreatorInput:
        ...
        return CreatorInput(name=author.name.strip(), creatorType="author")

    def _creator_inputs_to_zotero(self, creators: list[CreatorInput]) -> list[dict[str, str]]:
        normalized: list[dict[str, str]] = []
        for creator in creators:
            display_name = self._creator_input_display_name(creator)
            if not display_name:
                continue
            if creator.name:
                normalized.append(
            ...
            normalized.append(
                {
                    "firstName": (creator.firstName or "").strip(),
                    "lastName": (creator.lastName or "").strip(),
                    "creatorType": creator.creatorType,
                }
            )
        return normalized
    ...
    @staticmethod
    def _creator_input_display_name(creator: CreatorInput) -> str | None:
        if creator.name and creator.name.strip():
            return creator.name.strip()
        display_name = " ".join(
            part.strip()
            for part in [creator.firstName or "", creator.lastName or ""]
    ...
    def _ai_note_payload_fingerprint(self, payload: UpsertAINoteRequest) -> str:
        canonical_payload = {
            ...
            "schemaVersion": payload.schemaVersion or None,
            "payload": payload.payload or None,
            "provenance": [
                entry.model_dump(exclude_none=True) for entry in self._effective_provenance(payload)
            ],
        }
        raw = json.dumps(
            canonical_payload,
    ...
    def _effective_provenance(self, payload: UpsertAINoteRequest) -> list[ProvenanceRecord]:
        if payload.provenance:
            return list(payload.provenance)
        if payload.sourceAttachmentKey:
            return [
                ProvenanceRecord(
                    attachmentKey=payload.sourceAttachmentKey,
                    cursorStart=payload.sourceCursorStart,
                    cursorEnd=payload.sourceCursorEnd,
                )
            ]
        return []
    ...
    def _raise_request_id_conflict(self) -> NoReturn:
        raise BridgeError(
    ...
    def _raise_item_update_conflict(self) -> NoReturn:
        raise BridgeError(
            code="WRITE_CONFLICT",
